...
function Navbar() {
  ...
  const connectWallet = async (walletType) => {
    ...
    if (result) {
      ...
      navigator.vibrate([100,300]);
      ErrorToast(`Wallet is not found 💔`);
    }
  };
  ...
  return (
    <nav id="navbar" className="sticky z-50 sm:top-5">
      ...
      <div className="w-full bg-darkBlue-600g rounded-b-lg sm:rounded-lg bg-darkBlue-500 justify-between flex flex-wrap items-center mx-aut p-4">
        ...
        <ul
          id="nav-links"
          className="flex translate-y-[-40pc] flex-col top-[80%] gap-3 left-0 right-0 p-4 md:relative absolute md:p-0 mt-4 font-medium border-gray-100 bg-gray-50 md:translate-y-[0pc] md:gap-0  md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-darkBlue-500 md:dark:bg-transparent rounded-lg borderw dark:border-gray-700 text-sm md:text-base"
        >
          <li>
            <NavLink
              to="/"
        ...
        <div className="flex justify-between gap-4 items-center md:justify-center md:w-auto">
          {AccountState.isConnect ? (
            ...
            <>
              <button
                type="button"
                className="py-2 px-3 xs:py-2.5 xs:px-5 text-lg font-medium flex items-center gap-4 text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-purple-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-purple-600 dark:text-gray-100 border-none dark:hover:text-white dark:hover:bg-purple-700 bg-gradient-to-r from-purple-800 to-pink-600 "
                onClick={() => connectWallet('MetaMask')}
              >
               <img width="30" height="30" src="https://img.icons8.com/external-vitaliy-gorbachev-flat-vitaly-gorbachev/58/external-wallet-cryptocurrency-vitaliy-gorbachev-flat-vitaly-gorbachev.png" alt="external-wallet-cryptocurrency-vitaliy-gorbachev-flat-vitaly-gorbachev"/>Connect Wallet
              </button>
            </>
          )}
        </div>
