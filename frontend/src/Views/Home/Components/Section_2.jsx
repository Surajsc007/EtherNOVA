...
function Section_2() {
  ...
  useEffect(() => {
    const fetching = async () => {
      try {
        const response = await fetchAllNFTs("", "", 15);
        // Shuffle the array of NFTs
        const shuffledNFTs = response.sort(() => Math.random() - 0.5);
        setTradingNFTs(shuffledNFTs);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    >
      <div className="flex sm:flex-row flex-col gap-4 justify-between items-center">
        <h1 className="dark:text-white/80 text-2xl font-semibold sm:text-4xl">
          Trending{" "}
          <b className="border-2 sm:text-4xl text-lg font-semibold text-white lg:shadow-none lg:shadow-purple-800/80 sm:border-0 rounded-md shadow-lg shadow-purple-800/80 p-1 bg-gradient-to-r from-purple-600 via-pink-600 to-blue-500">
            NFTs
          </b>
        </h1>
      </div>
